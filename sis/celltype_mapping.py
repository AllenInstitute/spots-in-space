...
class MapMyCells(CellTypeMapping):
    """
    Mapping using the command line interface of MapMyCells documented here https://github.com/AllenInstitute/cell_type_mapper/blob/main/docs/mapping_cells.md
    This class prepares input configuration files for mapping, runs mapping, and loads in results for analysis. Support functions to make the necessary
    reference files to map against if the aren't already constructed.

    """

    def __init__(self, sp_anndata: Path|str, save_path: Path|str, meta: dict={}):
        """
        Paramaters
        ----------
        sp_anndata: Path|str
            file path to spatial transcriptomics anndata object
        save_path: Path|str
            directory to save mapping results. A unique folder will be created within this directory to store results and set to self.run_directory
        """

        self.ad_sp = None
        if type(sp_anndata) == str:
            self.ad_sp_file = Path(sp_anndata)
        else:
            self.ad_sp_file = sp_anndata

        self.meta = {'mapping method': 'MapMyCells'}
        self.meta.update(meta)
        self._create_run_directory(save_path)
        self.save_mapping(replace=True)

    def make_mapping_config_json(self, flatten: bool, precomputed_stats: Path|str|None=None, query_markers: Path|str|None=None, 
                         type_assignment: dict={}, args: dict={}):
        
        """
        Create a json file to pass to MapMyCells command line interface. This file will specify the parameters for mapping.

        Parameters
        ----------
        flatten: bool
            If True, will perform flat correlation mapping to all genes in `query_markers` file. If False, will perform hierarchical mapping.
        precomputed_stats: Path|str|None, default None
            path to precomputed_stats.h5 file. If None will use self.precomputed_stats_file
        query_markers: Path|str|None, default None
            path to query_markers.json file. If None will use self.query_markers_file
        type_assignment: dict, default {}
            dictionary of values for type_assignment. If not specified will use the following defaults:
            {'normalization': 'raw',
            'bootstrap_iteration': 1 if flatten else 100,
            'bootstrap_factor': 1.0 if flatten else 0.25,
            'n_runners_up': 0 if flatten else 3,
            'chunk_size': 5000,
            'n_processors': 32}
        args: dict, default {}
            additional arguments to pass to the mapping config json file

        """

        assert precomputed_stats is not None or hasattr(self, 'precomputed_stats_file'), 'Must specify a path for a precomputed_stats.h5 file or generate one with make_precomputed_stats'
        assert query_markers is not None or hasattr(self, 'query_markers_file'), 'Must specify a path for a query_markers.json file or generate one with make_query_markers'

        if precomputed_stats is None:
            precomputed_stats = self.precomputed_stats_file

        if type(precomputed_stats) == str:
            precomputed_stats = Path(precomputed_stats)
        self.precomputed_stats_file = precomputed_stats

        if query_markers is None:
            query_markers = self.query_markers_file

        if type(query_markers) == str:
            query_markers = Path(query_markers)
        self.query_markers_file = query_markers
        
        type_assignment_default = {
            'normalization': 'raw',
            'bootstrap_iteration': 1 if flatten else 100,
            'bootstrap_factor': 1.0 if flatten else 0.25,
            'n_runners_up': 0 if flatten else 3,
        ...
        type_assignment_default.update(type_assignment)

        config_dict = {
            'query_path': self.ad_sp_file.as_posix(),
            'extended_result_path': self.run_directory.joinpath('extended_results.json').as_posix(),
            'csv_result_path': self.run_directory.joinpath('mapmycells_results.csv').as_posix(),
            'flatten': flatten,
            'precomputed_stats': {'path': precomputed_stats.as_posix()},
            'query_markers': {'serialized_lookup': query_markers.as_posix()},
            'type_assignment': type_assignment_default,
        }

        config_dict.update(args)

        config_file = self.run_directory.joinpath('mapping_config.json')
        self.mapping_config_file = config_file
        self.save_mapping(replace=True)

        with open(config_file, 'w') as f:
            json.dump(config_dict, f, indent=2)

    def make_precomputed_stats(self):
        """Make precomputed_stats.h5 file for mapping."""
        raise NotImplementedError

    def make_query_markers(self):
        """Make query_markers.json file for mapping."""
        raise NotImplementedError
    
    def validate_h5ad(self, ad_sp: ad.AnnData):
        """Validate that AnnData object is compatible with MapMyCells."""
        raise NotImplementedError
    
    def run_on_hpc(self, ad_map_args:dict={}, hpc_args:dict={}):
        """
        Run mapping on HPC using MapMyCells command line interface.

        Parameters
        ----------
        ad_map_args: dict, default {}
            arguments to pass to make_mapping_config_json
        hpc_args: dict, default {}
            arguments to pass to run_slurm
        ...
        self.make_mapping_config_json(**ad_map_args)

        job_path = hpc_args.get('job_path', None)
        if job_path is None:
            print('must specify a job path')
            return
        job_path = Path(job_path).as_posix()
        hpc_default = {
            'hpc_host': 'hpc-login',
            'job_path': job_path,
            'partition': 'celltypes',
        ...
        json_config = self.mapping_config_file.as_posix()

        # Note this is currently using hard coded environments set up by Scott Daniel, hopefully at some point
        # this may get formalized into a docker container
        command = f"""
        # set environment variables to prevent independent processes
        # from internally parallelizing and competing for CPUs
        source /home/scott.daniel/single_core.sh

        # setup environment with cell_type_mapper installed in it
        source /home/scott.daniel/technology_conda3_cmd.sh
        conda activate cell_type_mapper

        python -m cell_type_mapper.cli.from_specified_markers --input_json {json_config}
        """
    ...
    def load_mapping_config_json(self):
        if not hasattr(self, 'mapping_config_file'):
            print('No attribute "mapping_config_file"')
            return
    ...
    def load_mapping_results(self):
        ...
        csv_results_file = Path(config_dict['csv_result_path'])
        if not csv_results_file.is_file():
            print(f'{csv_results_file} does not exist, check that mapping completed successfully')
            return
        ...
        if extended_results_file.is_file():
            ...
            self.ad_map.uns['extended_results'] = extended_results
