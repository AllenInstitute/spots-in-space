...
def run_cell_polygon_calculation(load_func, load_args:dict, subregion: str|tuple|None, cell_subset_file:str|None, result_file:str|None, alpha_inv_coeff: float=1):
...
class SpotTable:
    ...
    @staticmethod
    def _read_stereoseq_gem(gem_file, gem_cols, cell_col, skiprows, max_rows):
        ...
        if cell_col is not None:
            return raw_data, pos, gene_ids, gene_id_to_name, cell_ids
        
        else: 
            return raw_data, pos, gene_ids, gene_id_to_name
    ...
    def load_stereoseq(cls, gem_file: str|None=None, cache_file: str|None=None, gem_cols: dict|tuple=(('gene', 0), ('x', 1), ('y', 2), ('MIDcounts', 3)), 
        ...
        if cache_file is None or not os.path.exists(cache_file):
            print('Loading gem...')
            pos, gene_ids, gene_id_to_name = SpotTable._read_stereoseq_gem(gem_file, gem_cols, cell_col, skiprows, max_rows)
    ...
    @classmethod
    def load_npz(cls, npz_file, images=None):
...
class SegmentedSpotTable(SpotTable):
    """Represents a spatial transcriptomics spot table in which cells have been
    segmented.

    - Contains x, y, z, gene per spot
    - May be a subset of another table
    ...
    gene_names : numpy.ndarray, optional
        Array of shape (N,) describing the gene detected in each transcript, as
        an index into *gene_id_to_name*. Must specify either *gene_names* or 
        *gene_ids*, not both.
    gene_ids : numpy.ndarray, optional
        Array mapping from values in *gene_ids* to string names.
    parent_table : sis.spot_table.SpotTable, optional
        Indicates that this table is a subset of a parent SpotTable.
    parent_inds : numpy.ndarray, optional
        Indices used to select the subset of spots in this table from the
        parent spots.
    parent_region : tuple, optional
        X, Y boundaries ((xmin, xmax), (ymin, ymax)) used to select this table
    ...
    @classmethod
    def from_spot_table(cls, spot_table: SpotTable, cell_ids: np.array, **kwargs):
        """Create a SegmentedSpotTable from an existing (unsegmented) SpotTable
        and an array of integer cell_ids."""
        return cls(
                pos=spot_table.pos,
                gene_names=spot_table.gene_names,
                parent_table=spot_table.parent_table,
                parent_inds=spot_table.parent_inds,
                parent_region=spot_table.parent_region,
                images=spot_table.images,
                cell_ids=cell_ids,
                **kwargs
                )

    @classmethod
    def load_npz(cls, npz_file, images=None, allow_pickle=False):
        fields = np.load(npz_file, allow_pickle=allow_pickle)
        return cls(**fields, images=images)
    ...
    def copy(self, deep:bool=False, **kwds):
        ...
        return SegmentedSpotTable(**init_kwargs) 
