...
class SegmentationRun:
    ...
    def set_intermediate_file_paths(self):
        ...
        self.run_spec_path = output_dir.joinpath('run_spec.pkl')
    ...
    def run(self, overwrite=False):
        """Run all steps in a new subdirectory."""
        if not overwrite:
            ...
            if any([f.exists() for f in intermediate_file_paths]):
                raise RuntimeError('One or more files generated by this pipeline already exists in the defined output directory. '
                        'Run resume() to resume from a previous segmentation or run(overwrite=True) to overwrite existing results.')

        self.output_dir.mkdir(exist_ok=True)
        self.save_metadata()

        tiles, regions = self.tile_seg_region()
        run_spec = self.get_seg_run_spec(regions)
    ...
    def get_seg_run_spec(self, regions=None):
        if regions is None:
            regions = self.load_regions()
    ...
    def submit_seg_jobs(self, run_spec=None):
        if run_spec is None:
            run_spec = self.load_run_spec()
    ...
    def merge_segmented_tiles(self, run_spec=None):
        if run_spec is None:
            run_spec = self.load_run_spec()
    ...
    def create_cell_by_gene(self, spot_table=None, remove_bg=True):
